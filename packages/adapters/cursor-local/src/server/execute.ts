...
function renderSubstaffEnvNote(env: Record<string, string>): string {
  const substaffKeys = Object.keys(env)
    .filter((key) => key.startsWith("SUBSTAFF_"))
    .sort();
  if (substaffKeys.length === 0) return "";
  return [
    "Substaff runtime note:",
    `The following SUBSTAFF_* environment variables are available in this run: ${substaffKeys.join(", ")}`,
...
export async function ensureCursorSkillsInjected(
  ...
  const skillsDir = options.skillsDir ?? await resolveSubstaffSkillsDir();
  if (!skillsDir) return;
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[substaff] Failed to prepare Cursor skills directory ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  try {
    ...
    await onLog(
      "stderr",
      `[substaff] Failed to read Substaff skills from ${skillsDir}: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    return;
  }
  ...
  for (const entry of entries) {
    ...
    try {
      ...
      await onLog(
        "stderr",
        `[substaff] Injected Cursor skill "${entry.name}" into ${skillsHome}\n`,
      );
    } catch (err) {
      await onLog(
        "stderr",
        `[substaff] Failed to inject Cursor skill "${entry.name}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
}
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const promptTemplate = asString(
    ...
    "You are agent {{agent.id}} ({{agent.name}}). Continue your Substaff work.",
  );
  ...
  if (runtimeSessionId && !canResumeSession) {
    await onLog(
      "stderr",
      `[substaff] Cursor session "${runtimeSessionId}" was saved for cwd "${runtimeSessionCwd}" and will not be resumed in "${cwd}".\n`,
    );
  }
  ...
  if (instructionsFilePath) {
    try {
      ...
      await onLog(
        "stderr",
        `[substaff] Loaded agent instructions file: ${instructionsFilePath}\n`,
      );
    } catch (err) {
      const reason = err instanceof Error ? err.message : String(err);
      await onLog(
        "stderr",
        `[substaff] Warning: could not read agent instructions file "${instructionsFilePath}": ${reason}\n`,
      );
    }
  }
  const commandNotes = (() => {
  ...
  const substaffEnvNote = renderSubstaffEnvNote(env);
  const prompt = `${instructionsPrefix}${substaffEnvNote}${renderedPrompt}`;
  ...
  if (
    ...
    await onLog(
      "stderr",
      `[substaff] Cursor resume session "${sessionId}" is unavailable; retrying with a fresh session.\n`,
    );
