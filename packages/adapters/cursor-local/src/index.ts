...
Use when:
- You want Substaff to run Cursor Agent CLI locally as the agent runtime
...
- Substaff auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$substaff" and related skills on local runs.
- Substaff auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
