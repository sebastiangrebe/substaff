...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@substaff/db";
import {
  expandHomePrefix,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" substaff db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveSubstaffInstanceId());
  const configuredDir = opts.dir?.trim() || defaultDir;
  const backupDir = resolveBackupDir(configuredDir);
  const retentionDays = normalizeRetentionDays(
    opts.retentionDays,
    30,
  );
  const filenamePrefix = opts.filenamePrefix?.trim() || "substaff";
