...
describe("SubstaffApiClient", () => {
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new SubstaffApiClient({
    ...
    expect(headers["x-substaff-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new SubstaffApiClient({ apiBase: "http://localhost:3100" });
