...
const DATA_DIR_OPTION_HELP =
  "Substaff data directory root (isolates state from ~/.substaff)";

program
  .name("substaff")
  .description("Substaff CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program
  ...
  .option("--run", "Start Substaff immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Substaff setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "substaff")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Substaff")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Substaff server API")
