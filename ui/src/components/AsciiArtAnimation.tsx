...
type SubstaffSprite = (typeof SUBSTAFF_SPRITES)[number];
...
function spriteSize(sprite: SubstaffSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = SUBSTAFF_SPRITES[(baseCol + baseRow) % SUBSTAFF_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = SUBSTAFF_SPRITES[Math.floor(Math.random() * SUBSTAFF_SPRITES.length)]!;
