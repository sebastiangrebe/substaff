...
import { agentRoleLabel, formatLabel } from "../lib/labels";
...
export function Agents() {
  ...
  return (
    <div className="space-y-8">
      ...
      {filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">{filtered.length} team member{filtered.length !== 1 ? "s" : ""}</p>
      )}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <span className="text-xs text-muted-foreground ml-2">
            {agentRoleLabel[node.role] ?? formatLabel(node.role)}
...
function LiveRunIndicator({
  ...
  return (
    <Link
      ...
      className={cn("flex items-center gap-1.5 px-2 py-0.5 rounded-full hover:bg-emerald-500/20 transition-colors no-underline", live.bg)}
    ...
    >
      ...
      <span className={cn("text-[11px] font-medium", live.text)}>
