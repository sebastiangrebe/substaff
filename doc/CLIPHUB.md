...
ClipHub is the public registry where people share, discover, and download Substaff company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Substaff what a package registry is to a programming language. Substaff already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Substaff — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Substaff instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Substaff spec. It contains:
...
**Install (fresh start):**
```
substaff install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Substaff instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Substaff, export your company as a template, then publish:

```
substaff export --template my-company
substaff publish cliphub my-company
```
...
ClipHub is a **separate service** from Substaff itself. Substaff is self-hosted; ClipHub is a hosted registry that Substaff instances talk to.
...
| **Substaff CLI** | `substaff install`, `substaff publish`, `substaff cliphub sync` — built into Substaff |
| **Substaff UI** | "Browse ClipHub" panel in the Substaff web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Substaff) |
| Backend | TypeScript + Hono (consistent with Substaff) |
| Database | PostgreSQL |
...
4. Run `substaff install cliphub:acme/lean-saas-shop`
5. Substaff creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Substaff until it works well
2. Export: `substaff export --template my-agency`
3. Publish: `substaff publish cliphub my-agency`
...
3. Install just that agent: `substaff install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Substaff

ClipHub is **not required** to use Substaff. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Substaff what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `substaff install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Substaff web UI
- [ ] `substaff cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Substaff's own philosophy)
