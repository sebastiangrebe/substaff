...
This spec defines how Substaff actually runs agents while staying runtime-agnostic.
...
1. Substaff is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Substaff manages lifecycle, and on exit Substaff parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `substaff.skill` (shared Substaff skill text block)
...
2. Preferred transport is env vars (`SUBSTAFF_*`) injected at runtime.
