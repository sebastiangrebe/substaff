...
This document defines the V1 design for a Substaff adapter that integrates with
...
4. synthesized stdout events for Substaff UI/CLI
...
3. **Agent callback auth to Substaff** uses a bootstrap exchange flow (no long-lived Substaff key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Substaff endpoints, not full SKILL.md prompt injection.
...
- `substaffPublicUrl` (optional override; else `SUBSTAFF_PUBLIC_URL` env)
...
## Substaff Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `SUBSTAFF_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.substaffPublicUrl`
2. `process.env.SUBSTAFF_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Substaff credentials in prompt text.

Flow:

1. Before launch/follow-up, Substaff mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Substaff validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Substaff API calls.
...
1. Prompt includes a compact instruction to fetch skills from Substaff.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/substaff`
   - `GET /api/skills/substaff-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <substaffPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Substaff run context.
...
5. `substaffPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Substaff skill endpoints |
| Substaff auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Substaff context.
3. Add richer PR metadata surfacing in Substaff UI.
