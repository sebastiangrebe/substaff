...
Agents need API keys to authenticate with Substaff. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Substaff
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Substaff generates a token and passes it directly to the agent
...
`SUBSTAFF_API_URL`, `SUBSTAFF_AGENT_ID`, `SUBSTAFF_COMPANY_ID`. We need to
add a `SUBSTAFF_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Substaff CLI
...
```
substaff auth login
# Opens browser -> user confirms -> token stored at ~/.substaff/credentials
```
...
managed by the Substaff server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Substaff.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Substaff SKILL.md (or a link to it)
   - What information Substaff needs from the agent (e.g. webhook URL, adapter
...
5. Substaff stores the pending registration.
...
7. On approval, Substaff provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Substaff issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.substaff.ing/skills/substaff/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Substaff should send you heartbeats",
...
1. Generate a Substaff invite link for the company.
...
4. A Substaff company member approves the new agent.
5. Substaff begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `SUBSTAFF_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `substaff auth login` for developer-managed remote agents.                                      |
