# Substaff Module System

## Overview

Substaff's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Substaff's API, UI, and data model | Yes |
...
modules/
  observability/
    substaff.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@substaff/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@substaff/core";
...
```
1. Scan modules/ directory for substaff.module.json manifests
...
```jsonc
// substaff.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@substaff/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@substaff/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:substaff/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.substaff.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm substaff store list                    # browse available modules and templates
pnpm substaff store install <module-id>     # install a module
pnpm substaff store import <template-id>    # import a company template
pnpm substaff store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Substaff instance | `server:started` |
...
4. **Module config** — `substaff.config.json` with per-module config, env var interpolation
...
10. **`@substaff/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
