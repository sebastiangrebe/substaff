...
Add a single storage subsystem for Substaff that supports:
...
- API serves attachment bytes through authenticated Substaff endpoints.
...
- Default local path is under instance root: `~/.substaff/instances/<instanceId>/data/storage`.
...
- `substaff onboard` writes a valid `storage` config block by default.
- `substaff configure --section storage` can switch between local and s3 modes.
...
- `substaff doctor` reports actionable storage status.
