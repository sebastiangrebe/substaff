...
Build a custom adapter to connect Substaff to any agent runtime.
...
2. Build environment with `buildSubstaffEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `substaff run --watch` using `picocolors`.
...
Make Substaff skills discoverable to your agent runtime without writing to the agent's working directory:
