# ClipHub: Marketplace for Substaff Team Configurations

> The "app store" for whole-company AI teams — pre-built Substaff configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Substaff-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Substaff company (agents, projects, governance) |
...
A complete Substaff company configuration:
...
Single-agent configurations designed to plug into a Substaff org:
...
Portable skill files that any Substaff agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Substaff's skill loading system
...
interface Listing {
  ...
  substaffVersionMin: string;     // Minimum Substaff version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Substaff company
...
| `POST` | `/api/listings/:id/install` | Install to Substaff company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Substaff company (or create new)
  → ClipHub API calls Substaff API to:
  ...
  → Redirect to Substaff dashboard with new team running
```
...
### 5.3 Creator: Export from Substaff → Publish

```
Running Substaff company → "Export as Blueprint" (CLI or UI)
  → Substaff exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Substaff brand blue/purple)
...
| Install flow | One-click deploy to Substaff company |
...
{
  "targetCompanyId": "uuid",         // Existing Substaff company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `substaff-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Substaff
- **Backend**: Node.js API (or extend Substaff server)
- **Database**: Postgres (can share Substaff's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Substaff auth (or OAuth2)

### 9.2 Integration with Substaff

ClipHub can be:
- **Option A**: A separate app that calls Substaff's API to install blueprints
- **Option B**: A built-in section of the Substaff UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Substaff UI and API.
...
- [ ] Install flow: blueprint → Substaff company
...
- [ ] "Export from Substaff" CLI command
