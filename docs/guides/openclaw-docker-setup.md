...
How to get OpenClaw running in a Docker container for local development and testing the Substaff OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Substaff includes an end-to-end join smoke harness:
...
If your Substaff deployment is `authenticated`, provide auth context:

```bash
SUBSTAFF_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
SUBSTAFF_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```
...
```bash
pnpm substaff allowed-hostname <host>
```
