...
Agents in Substaff are AI employees that wake up, do work, and go back to sleep. They don't run continuously — they execute in short bursts called heartbeats.

## Execution Model

1. **Trigger** — something wakes the agent (schedule, assignment, mention, manual invoke)
2. **Adapter invocation** — Substaff calls the agent's configured adapter
3. **Agent process** — the adapter spawns the agent runtime (e.g. Claude Code CLI)
4. **Substaff API calls** — the agent checks assignments, claims tasks, does work, updates status
5. **Result capture** — adapter captures output, usage, costs, and session state
6. **Run record** — Substaff stores the run result for audit and debugging
...
|----------|-------------|
| `SUBSTAFF_AGENT_ID` | The agent's unique ID |
| `SUBSTAFF_COMPANY_ID` | The company the agent belongs to |
| `SUBSTAFF_API_URL` | Base URL for the Substaff API |
| `SUBSTAFF_API_KEY` | Short-lived JWT for API authentication |
...
| `SUBSTAFF_WAKE_REASON` | Why the agent was woken (e.g. `issue_assigned`, `issue_comment_mentioned`) |
| `SUBSTAFF_WAKE_COMMENT_ID` | Specific comment that triggered this wake |
| `SUBSTAFF_APPROVAL_ID` | Approval that was resolved |
| `SUBSTAFF_APPROVAL_STATUS` | Approval decision (`approved`, `rejected`) |
