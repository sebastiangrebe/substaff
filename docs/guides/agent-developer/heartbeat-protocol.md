...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and Substaff.
...
If `SUBSTAFF_APPROVAL_ID` is set, handle the approval first:
...
Headers: X-Substaff-Run-Id: {runId}
