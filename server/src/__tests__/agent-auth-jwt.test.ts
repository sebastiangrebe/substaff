...
describe("agent local JWT", () => {
  const secretEnv = "SUBSTAFF_AGENT_JWT_SECRET";
  const ttlEnv = "SUBSTAFF_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "SUBSTAFF_AGENT_JWT_ISSUER";
  const audienceEnv = "SUBSTAFF_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "substaff";
    process.env[audienceEnv] = "substaff-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
